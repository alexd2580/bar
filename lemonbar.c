...
enum { GC_DRAW = 0, GC_CLEAR, GC_ATTR, GC_MAX };
...
void fill_gradient(xcb_drawable_t d, int x, int y, int width, int height, rgba_t start, rgba_t stop) {
    ...
    for(i = 0.; i < 1.; i += (1. / K)) {
        ...
        xcb_poly_fill_rectangle(c, d, gc[GC_DRAW], 1, (const xcb_rectangle_t[]){{x, i * bh, width, bh / K + 1}});
    }
 ...
 */
void draw_string(monitor_t* monitor, font_t* font, XftDraw* xft_draw, int x_offset, uint16_t* string, int num_chars) {
...
bool font_has_glyph(font_t* font, const uint16_t c) { return XftCharExists(dpy, font->xft_ft, (FcChar32)c) != 0; }
...
void add_button(draw_context_t* context, char** cursor_ptr, char* block_end) {
    ...
    char* asd = NULL;
    if(!area_add(*cursor_ptr, block_end, &asd, context->monitor, context->section_widths[context->align],
...
void parse_command(draw_context_t* context, char* cursor, char* command_end) {
    ...
    case 'l':
        ...
        break;
    case 'c':
        context->align = ALIGN_C;
        break;
    case 'r':
        context->align = ALIGN_R;
        break;

    case 'A':
        add_button(&context, cursor, command_end);
        break;

    // Set a color.
    case 'B':
        bgc = parse_color(cursor, dbgc);
        update_gc();
        break;
    case 'F':
        fgc = parse_color(cursor, dfgc);
        update_gc();
        break;
    case 'U':
        ugc = parse_color(cursor, dugc);
        update_gc();
        break;

    case 'S':
        select_monitor(&context, cursor);
        break;
    case 'O':
        offset_by_pixels(&context, cursor);
        break;
    case 'T':
        set_preferred_font_index(&context, cursor);
        break;
    ...
    void parse(char* cursor) {
        ...
        void set_ewmh_atoms(void) {
            ...
            for(monitor_t* mon = monhead; mon; mon = mon->next) {
                ...
                xcb_change_property(c, XCB_PROP_MODE_REPLACE, mon->window, atom_list[NET_WM_WINDOW_TYPE], XCB_ATOM_ATOM,
                                    32, 1, &atom_list[NET_WM_WINDOW_TYPE_DOCK]);
                ...
                xcb_change_property(c, XCB_PROP_MODE_REPLACE, mon->window, atom_list[NET_WM_DESKTOP], XCB_ATOM_CARDINAL,
                                    32, 1, (const uint32_t[]){0u - 1u});
                xcb_change_property(c, XCB_PROP_MODE_REPLACE, mon->window, atom_list[NET_WM_STRUT_PARTIAL],
                                    XCB_ATOM_CARDINAL, 32, 12, strut);
                xcb_change_property(c, XCB_PROP_MODE_REPLACE, mon->window, atom_list[NET_WM_STRUT], XCB_ATOM_CARDINAL,
                                    32, 4, strut);
                xcb_change_property(c, XCB_PROP_MODE_REPLACE, mon->window, XCB_ATOM_WM_NAME, XCB_ATOM_STRING, 8, 3,
                                    "bar");
        ...
        monitor_t* monitor_new(int x, int y, int width, int height) {
            ...
            uint32_t mask = XCB_CW_BACK_PIXEL | XCB_CW_BORDER_PIXEL | XCB_CW_OVERRIDE_REDIRECT | XCB_CW_EVENT_MASK |
                            XCB_CW_COLORMAP;
            uint32_t value_list[] = {bgc.v, bgc.v, dock, XCB_EVENT_MASK_EXPOSURE | XCB_EVENT_MASK_BUTTON_PRESS,
                                     colormap};
            xcb_create_window(c, depth, monitor->window, scr->root, monitor->x, monitor->y, width, bh, 0,
        ...
        void get_randr_monitors(void) {
            ...
            rres_reply = xcb_randr_get_screen_resources_current_reply(
                c, xcb_randr_get_screen_resources_current(c, scr->root), NULL);
            ...
            for(i = 0; i < num; i++) {
                ...
                oi_reply = xcb_randr_get_output_info_reply(
                    c, xcb_randr_get_output_info(c, outputs[i], XCB_CURRENT_TIME), NULL);
                ...
                ci_reply = xcb_randr_get_crtc_info_reply(
                    c, xcb_randr_get_crtc_info(c, oi_reply->crtc, XCB_CURRENT_TIME), NULL);
        ...
        int main(int argc, char** argv) {
            ...
            while((ch = getopt(argc, argv, "hg:bdf:a:pu:B:F:U:n:o:")) != -1) {
                ...
                case 'u':
                    ...
                    break;
                case 'o':
                    add_y_offset(strtol(optarg, NULL, 10));
                    break;
                case 'B':
                    dbgc = bgc = parse_color(&optarg_cpy, (rgba_t)0x00000000U);
                    break;
                case 'F':
                    dfgc = fgc = parse_color(&optarg_cpy, (rgba_t)0xffffffffU);
                    break;
                case 'U':
                    dugc = ugc = parse_color(&optarg_cpy, fgc);
                    break;
                case 'a':
                    areas = strtoul(optarg, NULL, 10);
                    break;
                }
            }
            ...
            if(areas) {
                ...
                if(!area_stack.area) {
                    fprintf(stderr,
                            "Could not allocate enough memory for %d clickable areas, try lowering the number\n",
                            areas);
                    return EXIT_FAILURE;
                }
            } else
